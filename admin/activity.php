...
$parentId = $_GET['parentId'];
	...
	if(isset($_POST['save']) && $_POST['save'] == "Save")
	{
		...
		if (empty($msg)) {
			$catId = $groups->saveActivity('', $name, $urlname, $block, $activity, $shortcontents, $contents, $weight, $featured, $pageTitle, $pageKeyword);
	...
	if(isset($_POST['edit']) && $_POST['edit'] == "Edit")
	{
		...
		if (empty($msg)) {
			$groups->saveActivity($_GET['eid'], $name, $urlname, $block, $activity, $shortcontents, $contents, $weight, $featured, $pageTitle, $pageKeyword);
...
<form action="<?= $_REQUEST['uri'];?>" method="post" onsubmit="return frmvalidate(this)" enctype="multipart/form-data">
        <table width="100%" border="0" cellspacing="2" cellpadding="0">
          ...
          <tr>
          	<th class="tahomabold11">Sub Category Of</th>
            <td>
              <select name="activity" style="width:200px">
				...
				{?>
				   <option value="<?=$actGet['name'];?>" >
						<? if($actGet['activity']!="Default") echo "--"; echo $actGet['name'];?>
				   </option>
				<? }?>
              </select>
            </td>
...
?>
            ...
            <td>
              <select name="activity" style="width:200px">
				...
				while($actGet=$conn->fetchArray($act))
				{?>
				   <option value="<?=$actGet['name'];?>" <? if($actGet['name']==$row['activity']){?> selected="selected" <? }?>>
						<? if($actGet['activity']!="Default") echo "--"; echo $actGet['name'];?>
				   </option>
				<? }?>
              </select>
  ...
  <tr align="left" <?php if ($counter % 2 == 0) { echo "bgcolor='#F1F1F1'";} ?>>
    <td><? if($catRow['activity']!="Default") echo "--";?>
		<?
        	$cat=$catRow['name']; $catid=$catRow['id']; $sql=mysql_query("select * from groups where activity='$cat'"); $data=mysql_fetch_array($sql);
			if($data['linkType']=='Product')
			{?>
				<a href="productsort.php?categoryid=<?=$catid;?>" style="font-size:11px; text-decoration:none"><? echo $catRow['name'];?></a>	
			<? }
			else
			{?>
				<p style="font-weight:bold"><?=$catRow['name'];?></p>		
			<? }
		?>
  	</td>
    <td><?=$catRow['activity'];?></td>
    <td><?=$catRow['block'];?></td>
    <td><?=$catRow['weight'];?></td>

...
class Groups
{
	...
	function writeBreadCrumb($id)
	{
		...
		echo "<a class='breadcrumb' href='home'>Home</a>"; echo "&nbsp;&raquo;&nbsp;";
		...
		$pr=$this->getById($id);
		$prGet=$conn->fetchArray($pr);
		echo "<a class='breadcrumb' href='". $prGet['urlname'] . "'>" . $prGet['name'] . "</a>";
		echo '</div>';
	}
	...
	//forum operations
	function saveForum($id, $contents, $userid, $publish, $weight)
	{
		global $conn;
		...
		$sql = "INSERT INTO forum SET contents='$contents',userId='$userid',publish = '$publish',weight = '$weight',onDate = NOW()";
		//echo $sql; die();
		$conn->exec($sql);
		if($id > 0)
			return $conn -> affRows();
	...
	//comment section
	function saveComment($comment, $postid, $userid, $publish)
	{
		global $conn;
	...
	function saveEmail($id, $subject, $contents, $weight)
	{
		global $conn;
		
		$id = cleanQuery($id);
		$subject= cleanQuery($subject);
		$contents= cleanQuery($contents);
		$weight=cleanQuery($weight);
		$sql=mysql_query("select max(id) as id from groups");
		$maxid=mysql_fetch_array($sql); $maxid=$maxid['id']+1;
	...
	function getLastWeightEmail()
	{
		global $conn;
		
		$sql = "SElECT weight FROM groups WHERE linkType='Email' ORDER BY weight DESC LIMIT 1";
		$result = $conn->exec($sql);
		$numRows = $conn -> numRows($result);
		if($numRows > 0)
		{
